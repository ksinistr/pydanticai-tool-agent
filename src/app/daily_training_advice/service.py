...
class DailyTrainingAdviceContextBuilderProtocol(Protocol):
    def build(self, settings: DailyTrainingAdviceSettings) -> DailyTrainingAdviceContext: ...


class DailyTrainingAdviceService:
    def __init__(
        self,
        agent: Agent[None, DailyTrainingAdviceStructuredOutput],
        context_builder: DailyTrainingAdviceContextBuilderProtocol,
...
def _build_prompt(context: DailyTrainingAdviceContext) -> str:
    payload = {
        ...
        "remaining_weather_hours": context.remaining_weather_hours,
    }
    ...
    return "\n\n".join(
        [
            ...
            (
                ...
                "Focus on the remaining part of the current local day. "
                "If today_summary.count is greater than zero, account for the completed training and do not prescribe the same load again."
            ),
...
def _render_report(report: DailyTrainingAdviceStructuredOutput) -> str:
    sections = [
        ("Data Freshness", report.data_freshness),
        ("Readiness", [f"{report.readiness_level.upper()}: {report.readiness_summary}"]),
        ("Completed Today", report.completed_today),
        ("Remaining Today", report.remaining_today),
...
def _setup_error_message(missing_variables: tuple[str, ...], language: str | None) -> str:
    joined = ", ".join(missing_variables) if missing_variables else "daily training advice settings"
    if _is_russian(language):
        return f"Совет по тренировке на день пока не настроен. Нужны значения: {joined}."
    return f"Daily training advice is not configured yet. Missing: {joined}."


def _timezone_error_message(language: str | None) -> str:
    if _is_russian(language):
        return "Совет по тренировке на день не настроен: USER_TIMEZONE задан некорректно."
    return "Daily training advice is not configured correctly: USER_TIMEZONE is invalid."


def _both_sources_failed_message(context: DailyTrainingAdviceContext) -> str:
    sources = [
    ...
    if _is_russian(context.language):
        return f"Совет по тренировке на день не смог получить свежие данные. {details}"
    return f"Daily training advice could not fetch fresh data. {details}"


def _intervals_failed_message(context: DailyTrainingAdviceContext) -> str:
    if _is_russian(context.language):
        message = [
            "Свежая готовность из Intervals.icu недоступна, поэтому остаток дня лучше оставить консервативным.",
            "Вторую интенсивную или длинную сессию сегодня лучше не добавлять, пока нет нормальной оценки восстановления.",
        ]
    ...
    message = [
        "Intervals.icu readiness data is unavailable, so the rest of today should stay conservative.",
        "Avoid adding a hard or long second session until recovery data is available again.",
    ]
...
def _weather_line(context: DailyTrainingAdviceContext, russian: bool) -> str | None:
    if not context.has_weather() or not context.remaining_weather_hours:
        return None

    first_hour = context.remaining_weather_hours[0]
    ...
    if russian:
        return (
            f"По погоде ближайшее оставшееся окно начинается около {time_value}: "
            f"{weather}, {temp}°C, ветер {wind} км/ч."
        )
    return (
        f"Weather at the next remaining window around {time_value}: "
        f"{weather}, {temp}C, wind {wind} km/h."
    )


def _calendar_line(context: DailyTrainingAdviceContext, russian: bool) -> str | None:
